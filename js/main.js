(function () {
  ...
  function openProduct(id) {
    ...
    var similarHtml = similar.length > 0 ? `
      <div class="modal-similar">
        <h3 class="modal-similar-title">Похожие товары</h3>
  ...
  // Hero: показ картинки или заглушки
  ...
  // Hero: градиент с откликом на мышь
  ...
  // Отзывы: кнопки и авто-прокрутка
  ...
  if (reviewsWrap && reviewsTrack) {
    ...
    function startAutoScroll() {
      ...
      autoScroll = setInterval(function () {
        reviewsWrap.scrollLeft += 1;
        if (reviewsWrap.scrollLeft >= reviewsTrack.offsetWidth / 2) {
    ...
    reviewsWrap.addEventListener('mouseenter', stopAutoScroll);
    reviewsWrap.addEventListener('mouseleave', startAutoScroll);
    startAutoScroll();
    document.getElementById('reviewsPrev').addEventListener('click', function () {
      stopAutoScroll();
      reviewsWrap.scrollBy({ left: -cardStep, behavior: 'smooth' });
      setTimeout(startAutoScroll, 3000);
    });
    document.getElementById('reviewsNext').addEventListener('click', function () {
      stopAutoScroll();
      reviewsWrap.scrollBy({ left: cardStep, behavior: 'smooth' });
      if (reviewsWrap.scrollLeft >= reviewsTrack.offsetWidth / 2) reviewsWrap.scrollLeft = 0;
      setTimeout(startAutoScroll, 3000);
    });
  }
})();
